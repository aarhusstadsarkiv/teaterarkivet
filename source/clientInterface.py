...
class Client:
    ...
    def get_resource_v3(
        self, collection, resource_id, include_relations=True, fmt=None
    ):
        ...
        def format_relations(relations):
            ...
            return [
                {"label": u"Sceneroller", "data": onstage_sorted},
                {"label": u"Produktionshold", "data": offstage_sorted},
            ]
        ...
        if payload.get("status_code") == 0:
            ...
            if fmt == "json":
                ...
                if include_relations and payload["result"].get("relations"):
                    response["resource"]["relations"] = format_relations(
                        payload["result"].get("relations")
                    )
    ...
    # Base methods
    def get_resource(self, collection, resource_id, include_relations=True, fmt=None):
        """Fetch a single resource.
        ...
        if payload.get("status_code") == 0:
            ...
            if fmt == "json":
                ...
                # Linkify the filter-links and update any deprecated key-names
                response["resource"] = format_metadata(
    ...
    def get_collection(self, collection, query_params=None):
        ...
        def generate_limits(params=None, limit=None):
            ...
            for e in limit_options:
                ...
                if limit and (e == limit):
                    ...
                    current["add_link"] = self._urlencode(
                        non_limit_params + [("limit", e)]
                    )
                output.append(current)
            ...
            for tup in client_params:
                ...
                if key in self.filters and self.filters[key]["filter"] is True:
                    ...
                    if key == "collection":
                        new_params = [
                            (e[0], e[1])
                            for e in client_params
                            if e != tup and e[0] in self.filters and e[0] != "series"
                        ]
                    else:
                        new_params = [
                            (e[0], e[1])
                            for e in client_params
                            if e != tup and e[0] in self.filters
                        ]
                    current_filter["remove_link"] = self._urlencode(new_params)

                    # GENERATE VIEW-LINK
                    if self.filters[key]["type"] == "integer":
                        current_filter["view_link"] = (
                            self.filters[key]["endpoint"] + "/" + value
                        )
                    ...
                    if resolved_params and key in resolved_params.keys():
                        ...
                        current_filter["display_label"] = self.filters[key][
                            "display_label"
                        ]
                    else:
        ...
        (
            ...
            ids_only_bool,
            cursor_bool,
        ) = process_params(query_params, self.default_params)
        ...
        if payload.get("status_code") == 0:
            ...
            processed_query_params = [
                x for x in total_params if x not in self.default_params
            ]
            ...
            if self.default_params and payload.get("resolved_params"):
                resolved_params = format_resolved_params(
                    payload.get("resolved_params"), self.default_params
                )

            # Remove domain-part - and if hidden default_params, remove them
            if payload.get("next_page_url"):
                response["next_page"] = format_next_page_url(
                    payload.get("next_page_url"), self.default_params
                )

            # Generate filters from query_params
            if processed_query_params:
                response["filters"] = generate_filters(
                    processed_query_params, resolved_params
                )
    ...
    def get_collection_v2(self, query_params=[]):
        ...
        api_response = self.service.get_collection_v2(
            list(set().union(default_params, query_params))
        )
        ...
        if not api_response.get("errors"):
            ...
            return {"status_code": 1, "msg": api_response.get("errors")}
    ...
    def autocomplete(self, q):
        ...
        return self.service.autocomplete(
            [("t", q), ("auto_group", "2"), ("limit", "25")], decode=False
        )
    ...
    def generate_facets(self, query_params=None):
        def generate_facet(facet_file, facet_name, query_params=None):
                ...
                for item in facet_obj:
                    ...
                    if filterParams:
                        if currentParam in filterParams:
                            item["remove_link"] = self._generate_remove_link(
                                filterParams, currentParam
                            )
                        else:
                            item["add_link"] = self._generate_add_link(
            ...
            if query_params:
                filterParams = [
                    (e[0], e[1]) for e in query_params if e[0] in self.filters
                ]
            else:
                filterParams = None
        ...
        for facet in self.facets:
            ...
            output[facet.get("facet_label")] = generate_facet(
                facet_file, facet.get("facet_field"), query_params
            )
