...
class Service():
    ...
    def get_collection_v2(self, query_params):
        ...
        url = '?'.join(['https://aarhusarkivet.herokuapp.com/search', self._urlencode(query_params)])
    ...
    def get_resource(self, collection, resource_id, include_relations=True):
        ...
        if collection in ['records', 'records_v3']:
            ...
            url = '/'.join([self.BASE_URL, self.GET_COLLECTIONS.get(collection), resource_id])
    ...
    #         return {"status_code": 5, "status_msg": "You do not have the rights to create this type of resource."}

    #     if collection == 'relations':
    #         return {"status_code": 4, "status_msg": "Wrong method. Call 'insert_relation' to create a relation."}
    ...
    def insert_relation(self, post_params):
        ...
        def generate_relation(post_params):
            subject_id = subject_domain = object_id = object_domain = rel_label = rel_start = rel_end = None
            ...
            if subject_id and subject_domain and object_id and object_domain and rel_label:
                ...
                return [0, subject_id, object_id, [2, 3], rel_base_type, rel_label, rel_start, rel_end]
            else:
                return False
    ...
    def delete_resource(self, collection, resource_id):
        ...
        if collection in ['people', 'organisations', 'events', 'places', 'addresses', 'collections', 'relations']:
            return self._post_request(collection, operation="remove", resource_id=resource_id)
        else:
    ...
    def _post_request_v3(self, url, payload):
        ...
        response = requests.post(url, payload=payload, allow_redirects=False, headers={'Content-Type': 'application/x-www-form-urlencoded'})
        try:
            # response_to_dict = json.loads(response.content)
            # return response_to_dict
            response_to_dict = response.json()
